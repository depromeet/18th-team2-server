...
- `develop` 브랜치에서 커밋하려 하면 → **경고**: "develop 브랜치에 직접 커밋하고 있습니다. 계속하시겠습니까?"
- 브랜치명이 `feature/`, `fix/`, `chore/`, `refactor/`, `docs/`, `test/` 중 하나로 시작하지 않으면 → **경고**

### Step 3: 가드레일 — ktlint + 테스트 100%

**이 스킬은 아래 두 게이트를 모두 통과해야만 커밋 단계로 진행한다. 하나라도 실패하면 즉시 중단.**

#### 3-1. ktlint 검증 (필수, 100% 통과)
...
- 자동 수정 옵션을 안내:
  ...
  → ./gradlew ktlintFormat 실행 후 다시 시도하세요
  ```
- `ktlintFormat`을 자동 실행하지 않는다 — 사용자가 변경사항을 검토하고 다시 트리거하도록 한다
- `./gradlew ktlintCheck` 태스크가 없는 프로젝트면 **중단**하고 사용자에게 알림 (스킵 금지)

#### 3-2. 빌드 + 테스트 검증 (100% 성공 필수)
...
- 빌드 실패 시 → "빌드/테스트가 실패했습니다. 에러를 수정한 후 다시 시도하세요." 출력
- 에러 로그에서 핵심 실패 원인을 요약하여 제시
- 테스트 리포트(`build/reports/tests/test/index.html`) 경로 안내
- `@Disabled`, `@Ignore`로 스킵된 테스트가 신규로 추가되었으면 **경고** (의도적인 경우만 사용자 확인 후 진행)

#### 3-3. 가드레일 우회 금지

- ktlint·테스트 실패를 무시하고 커밋 진행 금지
...
- ktlintCheck 실패 상태에서 커밋 진행 금지
- 테스트 실패/스킵이 있는 상태에서 커밋 진행 금지
