...
- `nginx/conf.d/team2-active-upstreams.conf`는 nginx 직접 기동을 위한 503 fallback 파일이다. 배포 중에는 `scripts/deploy.sh` 또는 `scripts/deploy-nginx.sh`가 실행 중인 slot 기준으로 이 파일을 다시 렌더링한다.
