# Rolling Paper Write and Topping Lookup Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 초대 토큰 기반 롤링페이퍼 작성 API와 토핑 목록 조회 API를 추가한다. 작성 시 회원/비회원 participant를 생성 또는 복원하고, 작성자 닉네임 스냅샷과 토핑 선택값을 저장한다.
...
| `src/main/kotlin/com/team2/server/rollingpaper/controller/RollingPaperToppingApi.kt` | 토핑 목록 조회 Swagger 계약 |
| `src/main/kotlin/com/team2/server/rollingpaper/controller/RollingPaperToppingController.kt` | `GET /api/v1/rolling-paper-toppings` |
...
| `src/main/kotlin/com/team2/server/rollingpaper/dto/RollingPaperToppingResponse.kt` | 토핑 목록 응답 DTO |
| `src/main/kotlin/com/team2/server/rollingpaper/dto/RollingPaperToppingResult.kt` | 토핑 목록 UseCase 결과 DTO |
| `src/main/kotlin/com/team2/server/rollingpaper/repository/RollingPaperRepository.kt` | 롤링페이퍼 저장/중복 조회 |
| `src/main/kotlin/com/team2/server/rollingpaper/repository/RollingPaperToppingRepository.kt` | 토핑 조회 |
| `src/main/kotlin/com/team2/server/rollingpaper/usecase/CreateRollingPaperUseCase.kt` | 작성 트랜잭션 |
| `src/main/kotlin/com/team2/server/rollingpaper/usecase/GetRollingPaperToppingsUseCase.kt` | 토핑 목록 조회 |
| `src/test/kotlin/com/team2/server/rollingpaper/controller/RollingPaperControllerTest.kt` | 작성 API 통합 테스트 |
| `src/test/kotlin/com/team2/server/rollingpaper/controller/RollingPaperToppingControllerTest.kt` | 토핑 조회 API 통합 테스트 |
...
| `src/main/kotlin/com/team2/server/auth/config/SecurityConfig.kt` | 토핑 조회/롤링페이퍼 작성 공개 경로 추가 |
...
| 경로 | 변경 내용 |
|---|---|
| `src/main/resources/static/images/rolling-paper-wrappers/` | 기본 토핑 이미지 파일 배치 |
...
- [ ] Flyway seed migration에서 기본 토핑 row를 보장한다.
- [ ] `image(target_type = ROLLING_PAPER_WRAPPER, target_id = rolling_paper_wrapper.id, sort_order = 0)`를 보장한다.
- [ ] 이미지 URL은 `/images/rolling-paper-wrappers/...` 정적 경로를 사용한다.
- [ ] 런타임 initializer 없이 clean DB migration으로 기본 데이터와 각 기본 토핑의 이미지 row가 생성되는지 검증한다.
...
**Files:**
- Create: `src/main/kotlin/com/team2/server/rollingpaper/controller/RollingPaperToppingApi.kt`
- Create: `src/main/kotlin/com/team2/server/rollingpaper/controller/RollingPaperToppingController.kt`
- Create: `src/main/kotlin/com/team2/server/rollingpaper/dto/RollingPaperToppingResponse.kt`
- Create: `src/main/kotlin/com/team2/server/rollingpaper/dto/RollingPaperToppingResult.kt`
- Create: `src/main/kotlin/com/team2/server/rollingpaper/usecase/GetRollingPaperToppingsUseCase.kt`
- Modify: `src/main/kotlin/com/team2/server/auth/config/SecurityConfig.kt`
- Test: `src/test/kotlin/com/team2/server/rollingpaper/controller/RollingPaperToppingControllerTest.kt`

- [ ] `GET /api/v1/rolling-paper-toppings`를 추가한다.
- [ ] 응답은 `ApiResponse<List<RollingPaperToppingResponse>>`로 반환한다.
- [ ] 토핑 ID 오름차순으로 조회한다.
- [ ] image는 토핑 ID 목록으로 한 번에 조회하고, `sortOrder ASC` 첫 번째 URL을 매핑한다.
- [ ] 기본 토핑 이미지는 seed에서 보장하므로 `toppingImageUrl`은 non-null로 내려준다.
...
```bash
./gradlew test --tests com.team2.server.rollingpaper.controller.RollingPaperToppingControllerTest
```
...
- [ ] `ROLLING_PAPER_TOPPING_NOT_FOUND`, `ROLLING_PAPER_NICKNAME_DUPLICATED`, `ROLLING_PAPER_ALREADY_WRITTEN`를 추가한다.
...
```bash
./gradlew test --tests com.team2.server.rollingpaper.controller.RollingPaperToppingControllerTest --tests com.team2.server.rollingpaper.controller.RollingPaperControllerTest --tests com.team2.server.db.FlywayMigrationTest --tests com.team2.server.party.service.PartyInviteServiceTest
```
