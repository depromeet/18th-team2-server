...
클라이언트는 복귀할 **경로**(`returnPath`)만 넘긴다. 도메인은 서버가 `app.web-base-url` 에서 붙인다.
전체 URL 을 받아 화이트리스트로 거르는 방식도 검토했으나, 클라이언트가 호스트를 지정할 수 있게 하면
오픈 리다이렉트 방어가 전적으로 그 검증에 걸린다. 카카오에 넘기는 `redirect_uri` 는 어차피 서버 콜백으로
고정이라 호스트를 열어둘 이유가 없다. 경로만 받으면 다른 호스트로 나갈 방법 자체가 사라진다.
...
`GET /api/v1/kakao-calendar/consent?ticket={티켓}&return_path={프론트 복귀 경로}`

티켓 서명을 검증하고 만료를 확인한다. `return_path` 는 `/` 로 시작하고 `//`·`/\`·fragment·제어문자를
포함하지 않아야 한다. 둘 다 통과하면 티켓과 `return_path` 를 쿠키에 담고 카카오 인가 URL 로 리다이렉트한다.

`app.web-base-url` 은 기동 시 한 번 파싱해 정규화한다. 매 요청 검사로 미루면 설정 오타가 "복귀가
조용히 어긋나는" 형태로만 드러나므로, 잘못된 값이면 앱이 아예 뜨지 않게 한다.
...
인증 예외로 둔다. 다만 무방비는 아니다. 진입 경로는 서명된 티켓 검증과 `return_path` 형식 검증으로,
...
1. Controller가 저장된 연동에서 사용할 액세스 토큰을 확보한다. 만료가 임박했으면 리프레시로 갱신한다.
   연동이 없거나 갱신이 실패하면 `KAKAO_CALENDAR_CONSENT_REQUIRED` 로 끝낸다.
2. Controller가 `userId`, `partyId`, 확보한 토큰을 등록 UseCase에 넘긴다.
3. `PartyCalendarInfoPort`로 파티를 조회하고, 요청자가 그 파티의 호스트이거나 현재 참여 중인 참여자인지 검증한다. 파티를 나간 참여자(`hasLeft`)는 등록할 수 없다. 아니면 `PARTY_FORBIDDEN`.
4. 파티가 아직 시작되지 않았는지 확인한다. 현재 시각이 `startedAt` 이후면 `TALK_CALENDAR_PARTY_ALREADY_STARTED`.
5. 등록 이력을 조회한다.
   - 없으면 `event_id`가 빈 행을 먼저 INSERT하고 flush한다. 그 다음 카카오 일정 생성을 호출하고 받은 `event_id`를 채운다.
   - 있으면 카카오 일정 수정을 호출한다.
6. `event_id` 와 갱신 여부(`updated`)를 응답한다.

토큰 확보가 파티 검증보다 앞에 온다. 토큰이 등록 UseCase 의 커맨드 입력이라 먼저 확보해야 하기 때문이다.

대가가 있다. 연동이 없는 사용자가 존재하지 않는 파티, 이미 시작된 파티, 자신이 멤버가 아닌 파티에
등록을 시도하면 진짜 사유 대신 `KAKAO_CALENDAR_CONSENT_REQUIRED` 를 받는다. 동의를 마치고 재시도해야
비로소 404·409·403 을 본다. 최초 동의 이전 한 번만 마주치는 경로이고 데이터가 어긋나지도 않아 감수한다.

바로잡으려면 파티 검증을 컨트롤러나 선행 단계에서 한 번 더 수행해야 하는데, 그러면 정상 경로에서도
파티를 두 번 조회하게 된다. 동의가 없을 때만 선행 검증을 돌려 정확한 사유를 내려주는 방식이 절충안이며,
필요해지면 그때 넣는다.
...
편집/삭제(talk_calendar)" 다. 콘솔에서 이 동의항목을 선택 동의로 활성화하는 것은 배포 전 확인 항목으로 남아 있다.
