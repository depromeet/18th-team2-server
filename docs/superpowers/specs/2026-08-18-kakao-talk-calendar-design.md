...
직접 주면 `state` 쿠키와 티켓이 심기지 않아 콜백 검증이 반드시 실패한다. 클라이언트는 카카오 REST 키도
scope 이름도 알 필요가 없다.

티켓은 이 URL 에 포함되므로, 진입 엔드포인트가 사실상 인증되는 효과도 함께 얻는다.
...
필요한 동의항목은 **`talk_calendar`** 다. 문서상 "톡캘린더 및 일정 생성, 조회, 편집/삭제" 항목이다.
`talk_calendar_task` 는 톡캘린더 **할 일** 권한이라 이 기능과 무관하다. 초기 설계에 이 키를 잘못 적어
두었으므로 콘솔 설정과 인가 요청 모두 `talk_calendar` 로 맞춘다. 틀린 scope 로 인가를 요청하면
`KOE205` 계열로 실패한다.
...
등록한 뒤 파티에서 나가거나 파티가 삭제돼도 카카오 캘린더의 일정은 그대로 남는다. 일정 삭제 API 가
범위 밖이라 정리할 수단이 없다. 사용자가 직접 지울 수 있는 자기 일정이라 당장 문제는 아니지만,
일정 삭제를 붙일 때 함께 다뤄야 한다.
