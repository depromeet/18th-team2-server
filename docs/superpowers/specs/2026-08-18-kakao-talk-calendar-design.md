...
필요한 동의항목은 **`talk_calendar`** 로 본다. 문서상 "톡캘린더 및 일정 생성, 조회, 편집/삭제" 항목이다.
초기 설계의 `talk_calendar_task` 는 톡캘린더 할 일 권한으로 보이며 이 기능과 무관하다.

다만 이 구분은 문서 페이지를 읽어 얻은 것이고 콘솔 화면으로 확정한 것이 아니다. **구현 착수 전에 카카오
개발자 콘솔의 동의항목 화면에서 실제 키와 표시 문구를 확인해 여기에 못박는다.** 틀린 scope 로 인가를
요청하면 `KOE205` 계열로 실패하므로 첫 호출부터 막힌다.

같은 화면에서 그 항목이 앱 권한 신청이나 비즈 앱 전환을 요구하는지도 함께 확인한다. 요구한다면 심사 기간이
붙어 일정이 통째로 밀리므로, 코드보다 먼저 알아야 한다.
...
| 확인할 것 | 어디서 | 틀렸을 때 |
|---|---|---|
| 톡캘린더 동의항목의 실제 키 | 카카오 개발자 콘솔 동의항목 화면 | 인가 요청이 `KOE205` 로 첫 호출부터 실패 |
| 그 항목이 앱 권한 신청·비즈 앱 전환을 요구하는지 | 같은 화면 | 심사 기간만큼 일정이 밀림 |
| 삭제된 일정 수정 시 카카오 응답 코드 | 실제 호출 | 재생성 경로가 죽는데 정상 경로 테스트는 통과 |
