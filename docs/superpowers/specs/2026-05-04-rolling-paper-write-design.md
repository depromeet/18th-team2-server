# Rolling Paper Write and Topping Lookup API Design

- 작성일: 2026-05-04
- 기준 브랜치: `develop`
- 목적: 초대 링크 진입 후 롤링페이퍼 작성, 토핑 선택지 조회 API 계약 확정
...
토핑은 별도 조회 API로 내려주고, 작성 API는 `toppingId`를 받는다.

이유:

- 작성 요청에서 토핑 이름이나 이미지 URL을 받으면 서버가 실제 선택 가능한 토핑인지 검증하기 어렵다.
...
- 카드 표시용 `writerNickname`, `toppingImageUrl`은 리스트 조회 응답의 책임으로 둔다.
- 작성 API는 생성 성공 여부와 생성 ID만 책임진다.

외부 API와 코드 도메인 용어는 토핑 기준으로 정리하고, DB 물리명은 기존 `wrapper` 이름을 유지한다.

이유:

- 외부 API는 `toppingId`를 쓰는데 내부 FK가 `theme_id`이면 래핑지 개념과 테마 개념이 섞인다.
- 프론트 계약과 백엔드 코드에서는 제품 언어인 토핑을 사용한다.
- `rolling_paper_wrapper`, `wrapper_id`, `ROLLING_PAPER_WRAPPER`는 이미 migration과 seed에 잡힌 물리 이름이므로 이번 변경에서는 유지한다.
- 정적 이미지 URL의 저장 경로(`/images/rolling-paper-wrappers/...`)도 기존 seed 데이터와 파일 경로를 유지한다.
- JPA 매핑에서만 `RollingPaper.topping`이 `wrapper_id`를 바라보도록 번역한다.
...
5. 프론트가 `GET /api/v1/rolling-paper-toppings`로 선택 가능한 토핑 목록을 조회한다.
6. 사용자가 닉네임, 내용, 토핑을 모두 입력/선택한다.
...
- 필수
- 작성 요청은 `toppingId`를 받는다.
- 존재하지 않는 `toppingId`이면 실패
- 토핑 이미지 URL은 `image.target_type = ROLLING_PAPER_WRAPPER`, `image.target_id = rolling_paper_wrapper.id`, `sort_order ASC` 첫 번째 이미지를 사용한다.
...
| 필드 | 설명 |
|---|---|
| `toppingId` | 작성 요청의 `toppingId`로 전달할 토핑 ID |
| `name` | 토핑 이름 |
| `toppingImageUrl` | 토핑 이미지 URL. 기본 토핑 seed에서 보장한다. |
...
- 토핑 목록은 `RollingPaperToppingRepository.findAll(Sort.by(ASC, "id"))`로 조회한다.
- 토핑 이미지 URL은 토핑마다 개별 조회하지 않는다.
- `ImageTargetType.ROLLING_PAPER_WRAPPER`와 토핑 ID 목록으로 image를 한 번에 조회한 뒤, 메모리에서 `targetId` 기준으로 매핑한다.
- 같은 토핑에 이미지가 여러 개 있으면 `sortOrder ASC` 첫 번째 이미지를 사용한다.
...
- `PartyInvite` 조회, 만료/종료 검증, `RollingPaperTopping` 조회, participant 생성/복원, 롤링페이퍼 저장, `hasWrittenPaper = true` 갱신은 모두 같은 트랜잭션에서 처리한다.
...
`theme` 필드와 `theme_id` 컬럼은 `topping` 필드와 `wrapper_id` 컬럼으로 변경한다. DB 컬럼명은 기존 wrapper 물리명을 유지한다.
...
`RollingPaperTopping`은 현재 `name`만 가진다. 이미지는 공통 `image` 테이블에서 조회한다.
...
운영 prod 설정은 DB schema `validate` 기준이다. 이번 토핑 용어 변경은 DB table/column을 유지하므로 이름 변경만으로는 schema migration이 필요하지 않다.
...
3. 정적 이미지 URL 접근과 `GET /api/v1/rolling-paper-toppings` 응답을 검증한다.
...
│   ├── RollingPaperToppingApi.kt
│   └── RollingPaperToppingController.kt
├── dto/
│   ├── CreateRollingPaperRequest.kt
│   ├── CreateRollingPaperResponse.kt
│   ├── RollingPaperToppingResponse.kt
│   └── RollingPaperToppingResult.kt
├── repository/
│   ├── RollingPaperRepository.kt
│   └── RollingPaperToppingRepository.kt
├── service/
│   └── (기본 토핑 seed는 Flyway migration에서 관리)
└── usecase/
    ├── CreateRollingPaperUseCase.kt
    └── GetRollingPaperToppingsUseCase.kt
```
...
- 토핑 목록 조회에서는 N+1을 피하기 위해 토핑 ID 목록으로 image를 `IN` 조회한다. 이 ID는 DB 물리 테이블 `rolling_paper_wrapper.id`와 같다.
- 현재 `Image`는 `targetType`, `targetId` 기반 공통 이미지 테이블이라 `RollingPaperTopping`와 직접 JPA 연관관계가 없다. 이 구조에서는 fetch join보다 `ImageRepository`의 bulk 조회 메서드를 추가하는 방식이 더 단순하다.
- 작성 성공 응답은 생성 ID 전용 DTO를 사용한다.
- `RollingPaperToppingResult`는 UseCase 내부 결과 DTO, `RollingPaperToppingResponse`는 Controller 응답 DTO로 구분한다.
...
- 인증 없이 조회 성공
- `toppingId`, `name`, `toppingImageUrl` 응답
- 기본 토핑 이미지는 seed에서 보장하므로 `toppingImageUrl`은 non-null
