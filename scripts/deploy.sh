...
upstream_target() {
    ...
    if [ "$target" = "legacy" ]; then
        ...
        echo "app-$app_env-$target"
    else
        echo "app-$app_env-blue"
    fi
}
...
render_nginx_upstreams() {
    ...
    dev_upstream="$(upstream_target dev "$dev_target")"
    prod_upstream="$(upstream_target prod "$prod_target")"

    cat > "$ACTIVE_UPSTREAMS_FILE" <<EOF
# Generated by scripts/deploy.sh. Do not edit on the server.
map \$host \$api_upstream {
    api.hapalin.com http://$prod_upstream:8080;
    dev-api.hapalin.com http://$dev_upstream:8080;
    default "";
}

map \$host \$legacy_dev_upstream {
    default http://$dev_upstream:8080;
}
EOF
}
...
install_nginx_upstreams_for_running_container() {
    if docker ps --filter 'name=^/team2-nginx$' --format '{{.Names}}' | grep -qx 'team2-nginx'; then
        docker cp "$ACTIVE_UPSTREAMS_FILE" team2-nginx:/etc/nginx/conf.d/team2-active-upstreams.conf 2>/dev/null || true
    fi
}
...
verify_nginx_route() {
    ...
    if ! command -v curl &>/dev/null; then
        echo "WARN: curl is not installed on host. Skipping nginx route verification."
        return 0
    fi
